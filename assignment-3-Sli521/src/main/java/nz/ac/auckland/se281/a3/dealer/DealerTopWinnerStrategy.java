...
public class DealerTopWinnerStrategy implements DealerStrategy {
	 ...
	 */
	public DealerTopWinnerStrategy(BlackJack game) {
		this.game = game;
	}
	...
	public Action play(Hand hand) {
		...
		if (hand.getScore() >= game.getTopWinnerTarget().getHand().getScore() // checks if score is greater than target
				|| game.getTopWinnerTarget().getHand().isBust() // checks if target busted
